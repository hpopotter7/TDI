...
$sql = "SELECT Numero_evento, Nombre_evento, Cliente, DATE_FORMAT(Inicio_evento, '%d/%m/%Y'), DATE_FORMAT(Fin_evento, '%d/%m/%Y'), Destino, Sede, Ejecutivo, Disenio, Produccion, Facturacion, Estatus, id_evento  FROM eventos where Estatus='CERRADO' order by Numero_evento ";
...
if ($result = $mysqli->query($sql)) {
     while ($row = $result->fetch_row()) {
        ...
        $estatus=$row[11];
            ...
            $res=$res.'<tr><td><button id="'.$row[12].'" class="btn btn-info ver_solicitudes">'.$row[0].'</button></td><td>'.$row[1].'</td><td>'.$CLIENTE.'</td><td>'.$row[3].'</td><td>'.$row[4].'</td><td>'.$row[5].'</td><td>'.$row[6].'</td><td>'.$EJE.'</td><td>'.$DISE.'</td><td>'.$pro.'</td><td>'.$money.'</td></tr>';
