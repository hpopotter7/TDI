...
$sql2="select sum(Total) from solicitud_factura where id_evento=".$evento;
...
$COUNT=0;
$sql2="SELECT p.id_sol_factura, ROUND(sum(p.total),2) from partidas p, solicitud_factura s where s.id_evento=".$evento." and p.id_sol_factura=s.id_solicitud group by p.id_sol_factura";
if ($result = $mysqli->query($sql2)) {
  while ($row = $result->fetch_row()) {
    $COUNT++;
      $tbody=$tbody."<tr>
      ...
      $tbody=$tbody."
        ...
        $tbody=$tbody."
                ...
                <td class='text-center'>".moneda($row[1])."</td>
                <td><a href='solicitud_factura.php?id=".$row[0]."' target='_blank'><label class='btn btn-info btn_descargar_facturas'><i class='fa fa-download fa-2x' aria-hidden='true'></i></label></a></td>
              </tr>";
...
<table class='table table-user-information table-sm'>
                    ...
                    <tbody>
                      <tr>
                        <td class='text-center'>".moneda($suma)."</td>
                      </tr>
                    </tbody>
                    </table>
</div>
...
<table class='table table-user-information table-sm'>
                    ...
                    <tbody>
                      <tr>
                        <td class='text-center'>".moneda($facturacion)."</td>
                      </tr>
                    </tbody>
                    </table>
</div>
...
<table class='table table-user-information table-sm'>
                    ...
                    <tfoot>
                        ...
                        <th colspan='2' class='text-center'>".moneda($total_facturas)."</th>
                      </tr>
                      </tfoot>
                  </table></div>";
