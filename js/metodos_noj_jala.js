function inicio(){
  ...
  $("#odc_fecha").datepicker({
   ...
   $('#btn_notificaciones').click(function(e){
    e.preventDefault();
        $("#notificaciones").removeClass('slideInLeft'); 
        setTimeout(function(){
          $("#notificaciones").addClass('slideOutLeft');
          bandera_click=false;
        },1);  
        bandera_click=false;
 ...
 /* function ocultar(nombre){
                    ...
                    if(response.includes("No existe") || response.includes("Error de conexion")){
                      ...
                      console.log(response);
                       ...
                       $("#div_login").fadeOut("swing", function() {
                        ...
                        var tipos=arr[2].split("&");
                        var tipo="";
                        var r=0;
                        for(r=0;r<=tipos.length-1;r++){
                          switch(tipos[r]){
                            case "EJE":
                        ...
                        var p=tipo.split("*");
                        $('#tipo_perfil').html("<ul>");
                        for(r=0;r<=p.length-2;r++){
                           $('#tipo_perfil').append('<li><i class="fa fa-caret-square-o-right" aria-hidden="true"></i> '+p[r]+'</li>');
                        }
                        $('#tipo_perfil').append("</ul>");
                          if(arr[2].includes("ADM")){
                            $('.solo_admin').show();
  ...
  $('#txt_fecha_inicio_evento').datepicker()
    ...
    function ver_eventos_modificar(){
      ...
      var datos={
       ...
       $.ajax({
              ...
              success:  function (response) {
               ...
               $('#c_eventos_cerrar').html(response);
              }
            });
    }
    ...
    $.ajax({
          ...
          success:  function (response) {
              ...
              //
               var select = $('#c_disenio');
                select.html(select.find('option').sort(function(x, y) {
                  // to change to descending order switch "<" for ">"
                  return $(x).text() > $(y).text() ? 1 : -1;
                }));
              //
              var datos=$('#c_disenio').html();
              datos="<option value='vacio'>Selecciona un usuario...</option><option value='NA'>NA</option>"+datos;
              $('#c_disenio').html(datos);
          }
    });
  }
  function ver_usuarios_produccion(){//obtener los usuarios registrados
    ...
    $.ajax({
          ...
          success:  function (response) {
              ...
              //
               var select = $('#c_produccion');
                select.html(select.find('option').sort(function(x, y) {
                  // to change to descending order switch "<" for ">"
                  return $(x).text() > $(y).text() ? 1 : -1;
                }));
              //
              var datos=$('#c_produccion').html();
              datos="<option value='vacio'>Selecciona un usuario...</option><option value='NA'>NA</option>"+datos;
              $('#c_produccion').html(datos);
          }
    });
  }
  ...
  $("#c_usuarios").change(function(){
      if($(this).val()!=""){
        ...
        // ver notificaciones de tipo whatsapp
        /*
        ...
        */
        ...
        function ver_notificaciones(){ // si se abre por primera vez
          ...
          if(true){ 
            ...
            $("#notificaciones").addClass('animated slideInLeft');
            bandera_click=false;
            bandera_activo="si";
          }
        }
        function ver_notificaciones2(){ //si previamente ya se abrio
          ...
          if(true){ 
            $('#notificaciones').hide();
            $("#notificaciones").removeClass('slideOutLeft'); 
              setTimeout(function(){
                $('#notificaciones').fadeIn();
                $("#notificaciones").addClass('slideInLeft');
              },1);  
          }
        }
        ...
        $('#menu_crear_evento').click(function(e){
           ...
           $("#div_cortina").animate({top: '0px'}, 700); 
        ...
        $('#menu_cerrar_evento').click(function(e){
           e.preventDefault();
           ver_eventos_modificar(); //pendiente
           $("#div_cortina").animate({top: '0px'}, 700); 
           $('#div_usuarios').fadeOut();
           $('#div_alta_cliente').fadeOut();
           $('#div_alta_proveedores').fadeOut();
...
*/
        ...
        function descargar_formato(titulo){
            window.location="solicitud_pago.php?id=0&titulo="+titulo;

          /*else if(titulo=="VIATICOS"){
            window.location="solicitud_viaticos.php?id=0";
          }
          if(titulo=="REEMBOLSO"){
            window.location="solicitud_reembolso.php?id=0";
          }
          */
 ...
 $('#enviar_cambios_evento').click(function(){
    if($('#c_eventos_modificar').val()=="" || $('#area_modificaciones').val()==""){
      ...
      var datos={
          "evento": $('#c_eventos_modificar option:selected').text(),
      ...
      $('#txt_rfc').focusout(function(){
        //se valida que no exista es RC, si esta duplicado no se guarda
        var rfc=$(this).val();
        var datos={
          "rfc": rfc,
         ...
         $.ajax({
              ...
              success:  function (response) {
               if(response.includes("ya existe")){
                generate('error', "Este RFC ya sa registrado");    
                $('#txt_rfc').val("");        
               }
              }
            });
