...
/*
  ...
  function copiar_permisos(usuario_origen, usuario_destino, noty){
    ...
    $(".tab-content").delegate(".btn_evento_pendiente", "click", function(e) {
    e.preventDefault();
    var id=$(this).attr('id');
  ...
  function ver_panel_tab(){
    ...
    $.ajax({
      ...
      success:  function (response) {
        ...
        else if(response.includes("NINGUNO")){
          $('#div_panel').html("");
          $('#div_panel').hide();
          $('#div_panel').remove();
        }
  ...
  function ver_eventos_vacios(){    
    $('#resultado_solicitudes').hide();
    llenar_eventos_ver_solicitudes("0");
    $.ajax({
      url:   'ver_eventos_vacios.php',
      type:  'post',
      success:  function (response) {
        $('#tabla_vacios_body').html(response);
        $('#tabla_vacios').DataTable({
          dom: 'Bfrtip',
          buttons: [
              'pdf'
          ],
