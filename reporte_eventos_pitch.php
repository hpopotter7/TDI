...
$sql = "SELECT Numero_evento, Nombre_evento, Cliente, DATE_FORMAT(Inicio_evento, '%d/%m/%Y'), DATE_FORMAT(Fin_evento, '%d/%m/%Y'), Destino, Sede, Ejecutivo, Disenio, Produccion, Facturacion, Estatus  FROM eventos where Estatus='PITCH'order by Numero_evento ";
...
if ($result = $mysqli->query($sql)) {
     while ($row = $result->fetch_row()) {
        ...
        if($estatus=="ABIERTO"){
            ...
            $res=$res.'<tr style="
    background-color: white;color: black;"><td>'.$row[0].'</td><td>'.$row[1].'</td><td>'.$CLIENTE.'</td><td>'.$row[3].'</td><td>'.$row[4].'</td><td>'.$row[5].'</td><td>'.$row[6].'</td><td>'.$EJE.'</td><td>'.$DISE.'</td><td>'.$pro.'</td><td>'.$money.'</td></tr>';
        }
