                        ...
                        <div class="table-responsive mb-4 mt-4">
                            <table id="zero-config" class="table table-hover table-bordered table-sm" style="width:100%">
                                ...
                                <tbody>
                                   ...
                                   $where="";
                                   if($tipo=="A"){
                                    $sql = "SELECT Razon_Social, Nombre_comercial, rfc, Nombre_contacto, Telefono, Correo_contacto, id_cliente FROM clientes where Numero_cliente!='0' and Estatus='activo' order by Razon_Social ASC";
                                   }
                                   if($tipo=="B"){
                                    $sql = "SELECT Razon_Social, Nombre_comercial, rfc, Nombre_contacto, Telefono, Correo_contacto, id_cliente FROM clientes where Estatus='bloqueado' or Estatus='borrado' order by Razon_Social ASC";
                                   }
                                   if($tipo=="P"){
                                    $sql = "SELECT Razon_Social, Nombre_comercial, rfc, Nombre_contacto, Telefono, Correo_contacto, id_cliente FROM clientes where Numero_cliente='0' and Estatus='activo' order by Razon_Social ASC";
                                   }
                                   ...
                                   if ($result = $mysqli->query($sql)) {
                                        while ($row = $result->fetch_row()) {
                                           ...
                                           if($tipo=="P"){
                                                $res=$res.'<tr><td>'.$row[0].'</td><td>'.$row[1].'</td><td>'.$row[2].'</td><td>'.$row[3].'</td><td>'.$row[4].'</td><td>'.$row[5].'</td><td><i id="'.$row[6].'" data-toggle="tooltip" data-placement="top" title="Ver cliente" class="fas fa-eye fa-2x btn_ver_cliente" style="color:blue; cursor:pointer"></i></td></tr>';
                                            }
                                       }
                                       $result->close();
                                   }
...
   var n= new Noty({
  ...
  return n;
        ...
        $("#zero-config").delegate('.btn_ver_cliente','click',function(){
            var id_cliente=$(this).attr("id");
            location.href="registro_cliente.php?id="+id_cliente; 
        });
        ...
        $("#zero-config").delegate('.btn_bloquear','click',function(){
            ...
            var n2 = new Noty({
                ...
                Noty.button('Aceptar', 'btn btn-success', function () {
                    //alert($('input#example').val());
                    cambiar_estatus_cliente(id_cliente, "borrado", n2);
                }, {id: 'button1', 'data-status': 'ok'}),
