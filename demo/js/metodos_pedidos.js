  ...
  return outArray;
      ...
      if(v1=="vacio"){
        generate("warning", "Debe seleccionar un equipo.", 3000);
          ...
          generate("warning", "Debe ingresar una precio.", 3000);
      }
      else{
        var arr=v2.split("[");
        var producto=arr[1];
        ...
        id_tr="tr"+contador;
  			$('#cuerpo').append('<tr id="'+id_tr+'"><td>'+v1+'</td><td>'+producto+'</td><td>'+code+'</td><td>'+v3+'</td><td>'+precio+'<button type="button" id="btn'+id_tr+'" class="boton_borrar btn btn-danger btn-xs pull-right"><i class="fa fa-trash" aria-hidden="true"></i> Borrar</button></td></tr>');
        contador++;
  			$('#txt_cantidad').val('');
        $('#txt_nombre').val('');
        $('#txt_precio').val('');
...
function mayus(string) 
{
    return string.charAt(0).toUpperCase() + string.slice(1);
          ...
          var arr=result.split("\n");
           ...
           var imagen=ver_imagen(usuario);
           $('#label_nombre').html("<h3>"+usuario+' <img src="img/'+imagen+'.png" alt="" id="imagen_usuario"></h3>');
           $('#imagen_usuario').css("width","50px");
...
$("#txt_cantidad").keydown(function(event) {
     ...
     if (event.keyCode == 46 || event.keyCode == 8)    {
      ...
      $('#txt_precio').blur(function(){
        if(!$(this).val().includes('$')){

            $('#txt_precio').formatCurrency();
        }
